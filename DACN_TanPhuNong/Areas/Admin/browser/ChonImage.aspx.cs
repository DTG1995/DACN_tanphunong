...
namespace DACN_TanPhuNong.Areas.Admin.browser
{
    public partial class ChonImage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                NewDirectoryButton.OnClientClick = "var name = prompt('Nhập tên thư mục:'); if (name == null || name == '') return false; document.getElementById('" + NewDirectoryName.ClientID + "').value = name;";
            }
            idparent.Value = Request.QueryString["idparent"];
            ResizeMessage.Text = "";
        ...
        protected void SelectImage(object sender, EventArgs e)
        {
            ...
            OkButton.OnClientClick = OkButtonClick(ImageFolder + ImageList.SelectedValue.Replace("'", "\\'")); 
        }

        private string OkButtonClick(string val)
        {
            string id = idparent.Value;
            return " window.top.opener.document.getElementById('"+id+"').value = encodeURI('" + val + "');" +
                   " window.top.close();" +
